...
// TODO: replace with a settings screen later — for now, update this IP when the server's address changes
const SERVER_URL: &str = "http://192.168.1.95:3001";
...
async fn open_plaid_link(_app: tauri::AppHandle) -> Result<String, String> {
    let resp = reqwest::get(format!("{}/plaid/link-token", SERVER_URL))
        .await.map_err(|e| e.to_string())?;
    let json: serde_json::Value = resp.json().await.map_err(|e| e.to_string())?;
    let token = json["link_token"].as_str()
        .ok_or("No link_token")?.to_string();
    let url = format!("{}/plaid/link-page?token={}", SERVER_URL, token);
...
async fn get_plaid_accounts() -> Result<serde_json::Value, String> {
    let resp = reqwest::get(format!("{}/plaid/accounts", SERVER_URL))
        .await.map_err(|e| e.to_string())?;
...
async fn get_plaid_transactions() -> Result<serde_json::Value, String> {
    let resp = reqwest::get(format!("{}/plaid/transactions?limit=500", SERVER_URL))
        .await.map_err(|e| e.to_string())?;
    resp.json().await.map_err(|e| e.to_string())
}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(e) = orchestrator::run_startup(handle.clone(), SERVER_URL.to_string()).await {
